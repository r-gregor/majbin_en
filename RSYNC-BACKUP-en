...
if [ $# -eq 1 ]; then
	export CURRENT_YEAR_ENV=$1
else
	export CURRENT_YEAR_ENV=${todays_year}
fi

if [ $CURRENT_YEAR_ENV -ne ${todays_year} ]; then
...
if [ ! -d ${gUsrSpcp}/${gCurrYr} ]; then
	echo -e "No such directory: ${gUsrSpcp}/${gCurrYr}\n"
	exit
fi
